...
new Chartist.Line('#chart-with-area', {
    labels: labels,
    series: [series]  // 将yourDataArray替换为您的数据数组
}, {
    low: Math.min(...series) - 10,  // 根据数据数组中的最小值调整低值
    showArea: true,
    fullWidth: true,
    axisY: {
        onlyInteger: false  // 允许在Y轴上使用非整数（包括负值）
    },
    plugins: [
        Chartist.plugins.tooltip()
    ]
...
// You can also use the totalProfit variable if needed
